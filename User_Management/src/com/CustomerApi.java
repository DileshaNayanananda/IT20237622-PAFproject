...
public class CustomerApi extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String result = customerObj.insertCustomer(request.getParameter("user_name"),
				request.getParameter("email"), request.getParameter("address"),
				request.getParameter("phone_num"), request.getParameter("password"));
	...
	protected void doPut(HttpServletRequest request, HttpServletResponse response)
		...
		String result = customerObj.updateCustomer(param.get("hidcustomerIDSave").toString(),
				param.get("user_name").toString().toString().replace("+", " "),
				param.get("email").toString().toString().replace("%40", "@"),
				param.get("address").toString().toString().replace("+", " "),
				param.get("phone_num").toString().toString().replace("+", " "),
				param.get("password").toString().toString().replace("+", " ")
				);
	...
	protected void doDelete(HttpServletRequest request, HttpServletResponse response)
		...
		String result = customerObj.deleteCustomer(param.get("user_id").toString());
